...
function ProfileCard({ user, setUser }: { user: User; setUser: (u: User) => void }) {
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setUser(updatedUser);
      refreshUser(); // global context update
    } catch (error) {
  ...
  return (
    <div className="glass-card rounded-[2rem] p-8 flex flex-col items-center text-center h-full relative overflow-hidden">
      ...
      <div className="w-full space-y-3 z-10">
        ...
        <div className="pt-2">
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border ${user.is_student_verified
                  ...
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#2c333a] border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg overflow-hidden z-10">
                    {workspaces.map((ws) => (
                      <button
                        ...
                        className={`w-full px-4 py-3 text-left hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center justify-between ${ws.id === selectedWorkspaceId ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                          }`}
                      >
                        <div>
...
export const ProjectSelect: React.FC<ProjectSelectProps> = ({ user: initialAuthUser, onSelectProject, onLogout }) => {
  ...
  // Use global user context instead of local state for simple profile display
  const { user, refreshUser } = useUser();

  // 마이페이지 탭 전환 시 데이터 로딩 (fullUser는 마이페이지 상세 전용)
  useEffect(() => {
    if (currentView === 'mypage') {
      const fetchMyPageData = async () => {
        setMypageLoading(true);
        try {
          // getMyInfo includes the latest profile image and name
          ...
          // Also refresh global user context to stay in sync
          refreshUser();
        } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0F111A] text-gray-900 dark:text-white flex overflow-hidden font-sans">
          ...
          <div>
            ...
            <div className="space-y-2">
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition-all ${currentView === 'projects'
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition-all ${currentView === 'settings' && settingsTab === 'profile'
                  ? 'bg-white/50 dark:bg-white/10 text-blue-600 dark:text-white border border-white/20 shadow-sm backdrop-blur-sm'
                  : 'text-gray-600 dark:text-gray-400 hover:bg-black/5 dark:hover:bg-white/5'
                  }`}
              >
                <UserIcon size={18} className={currentView === 'settings' && settingsTab === 'profile' ? "text-blue-500" : ""} />
                <span>마이페이지</span>
              </button>

              <button
                onClick={() => setCurrentView('community')}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition-all ${currentView === 'community'
                  ? 'bg-white/50 dark:bg-white/10 text-blue-600 dark:text-white border border-white/20 shadow-sm backdrop-blur-sm'
                  : 'text-gray-600 dark:text-gray-400 hover:bg-black/5 dark:hover:bg-white/5'
                  }`}
              >
                <MessageSquare size={18} className={currentView === 'community' ? "text-blue-500" : ""} />
                <span>커뮤니티</span>
              </button>
            </div>
          </div>
      ...
      <main className="flex-1 overflow-y-auto relative z-10 flex flex-col">
        {currentView === 'community' ? (
          <CommunityBoard projectId={1} viewType="grid" />
        ) : (
                      ...
                      >
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold shadow-sm overflow-hidden">
                          {user?.profile_image ? (
                            ...
                            (user?.nickname || user?.name || initialAuthUser.name).slice(0, 2)
                          )}
                        </div>
                        <div className="flex flex-col mr-2">
                          <span className="text-xs font-bold text-gray-900 dark:text-gray-100">{user?.nickname || user?.name || initialAuthUser.name}</span>
                          <span className="text-[10px] text-gray-500 dark:text-gray-400">{user?.email || initialAuthUser.email}</span>
                        </div>
                      </div>
                      <button
                        onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)}
